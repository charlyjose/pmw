...
from app.api.models.route_plan import City, PlacementVisitLocations, RoutePlan, RoutePlanCity, Unit
...
def nearest_location_algorithm(city_list: List[PlacementVisitLocations], unit) -> RoutePlan:
    ...
    route_plan_city = [
        RoutePlanCity(
            id=id, placement_id=current_city.placement_id, address=current_city.address, coordinate=current_city.coordinate, distance=0
        )
    ]
    ...
    while unvisited_cities:
        ...
        route_plan_city.append(
            RoutePlanCity(
                id=id,
                placement_id=nearest_city.placement_id,
                address=nearest_city.address,
                coordinate=nearest_city.coordinate,
...
def get_route_plan(cities: List[PlacementVisitLocations], unit: Unit) -> RoutePlan:
    """
